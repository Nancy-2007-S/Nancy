...
export default function DashboardPage() {
  ...
  // Handle Redirection based on Auth + Profile state
  useEffect(() => {
    ...
    if (!user) {
      ...
      if (!userProfile || userProfile.exists === false || !userProfile.onboarding_completed) {
  ...
  if (authLoading || (user && !profileLoaded)) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#fcfdff]">
        ...
        <p className="text-slate-500 font-medium animate-pulse z-10 font-sans tracking-tight">Preparing your universe...</p>
      </div>
    );
  }
